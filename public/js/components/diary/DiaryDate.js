...
async function updateDiaryContent(today) {
  ...
  // 다이어리 항목을 HTML로 렌더링
  const diaryEntriesHTML =
    diaryDataArray.length > 0
      ? diaryDataArray
          .map((diaryData, index) => {
            const diaryId = diaryData._id;
            const diaryWriter = diaryData.user_id || "Unknown";
  ...
  // 다이어리 항목 업데이트
  const diaryEntriesElement = document.querySelector("#diary-entries");
  if (diaryEntriesElement) {
    diaryEntriesElement.innerHTML = diaryEntriesHTML; // Update only if the element exists
  }
}
